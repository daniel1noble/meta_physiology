...
data2 <- data %>% dplyr::select(!grep("Comment", colnames(data)))

# Have a look at frequency and stuff
data2 %>% janitor::tabyl(Q2.2_ContVariable)

# Just some simple plots. Ok, needs some cleaning looking at this. Studies often used multiple effects

data2 %>% janitor::tabyl(Q2.1_EffectUsed) %>%
  dplyr::summarise(SMD = dplyr::if_else(grepl("SMD", Q2.1_EffectUsed), n, 0),
            lnRR = dplyr::if_else(grepl("Response Ratio", Q2.1_EffectUsed), n, 0),
            r = dplyr::if_else(grepl("correlation", Q2.1_EffectUsed), n, 0),
            other = dplyr::if_else(grepl("unclear/other", Q2.1_EffectUsed), n, 0)) %>% colSums(.)
...
data2 %>%
  dplyr::mutate(Q2.4_short = factor(Q2.4_WeightedModel, labels = c('unclear/other', 'unweighted', 'weighted'))) %>%
  ggplot(aes(axis2 = Q2.2_ContVariable, axis1 = Q2.4_short)) +
  ggalluvial::geom_alluvium(aes(fill = Q2.4_short), width = 1/4) +
  theme_minimal() +
  theme(legend.position = 'none', panel.grid = element_blank(), axis.text.y = element_blank()) +
  scale_fill_manual(values = c('red','orange','blue')) +
  ggalluvial::geom_stratum(fill = 'black', colour = 'darkgrey', width = 1/5) +

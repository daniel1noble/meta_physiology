...
In equation \@ref(eq:lnRR), $M_{1}$ is the mean of group 1, often a control group, where as $M_{2}$ is the mean of group 2 (e.g., a treatment group). The mean of $X_{i}$ for group *i* can be any measurement type (e.g. a physiological rate, mass etc) so long as the measurement variable is ratio scale. Log transformation of this ratio makes the effect size normally distributed (most commonly assumed by meta-analytic models). Equation \@ref(eq:slnRR) is the analytical solution for *lnRR*'s sampling variance where, $SD_{1}^2$ and $SD_{2}^2$ is the sample variance in group 1 and 2, respectively and $N_{1}$ and $N_{2}$ is the sample size in group 1 and 2.
...
Comparative physiologists are interested in meta-analysing effects across a diversity of species, experimental approaches / designs and environments. While an overall meta-analytic mean may be of interest, more often than not, understanding the drivers of effect size heterogeneity will be the primary interest. However, obvious drivers of effect size heterogeneity, such as temperature and dosage differences across studies, may not really be of interest. Comparative physiologists are likely to want an effect size or analytic approach that allows them to remove these 'nuisances'. Here, we provide a set of tools to deal with 'nuisance heterogeneity' by 'reinventing' standard effect sizes or using meta-regression models. Of course, 'nuisance heterogeneity' might in of itself be the main interest, in which case standard meta-regression approaches will work sufficiently. However, in many cases, comparative physiologists might simply want to focus on bigger, more interesting biological drivers of effect size direction and magnitude. Estimating complex meta-regression models with a multitude of fixed effects might result, however, in more challenging model to interpret As such, a combined approach, where some of the known heterogeneity is dealt with by deriving a variant effect size and associated sampling variance, and other biologically interesting moderators are fit in a meta-regression model, might be desired. Indeed, this may even be a necessity to simplify the model. We hope the set of tools we describe provide clarification around different sources of heterogeneity; providing new ways to help comparative physiologists communicate effect measures more richly to guide our understanding and decisions around pressing global challenges.
...
<!--- we should probably do credit type contrbution stuff here just to make sure that you did most of the job and led the project--->

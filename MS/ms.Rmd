...
# I'm just listing a bunch here, but we should think about what we want to include. Here's an example of how we can make a table with all equations. Fun!
names <- c("Mean",
           "Variance",
           "Log Response Ratio",
...
eqns_yi <- c("X_{i}",
             ...
             "\\frac{\\left( X_{1} - X_{2}  \\right)}{sd_{p}}J",
             "\\frac{1}{2} log \\left( \\frac{1+r_{i}}{1-r_{i}} \\right)",
             "log \\left( \\frac{y_{i2}+0.5}{n_{i2} - y_{i2}+0.5} \\right) - log \\left( \\frac{y_{i1}+0.5}{n_{i1} - y_{i1}+0.5} \\right)")
#eqns_vi <- c()

table_fin <- data.frame(names, eqns_yi)

...
plot <- p1 + p2 + p3 + plot_annotation(tag_levels = c("A"), tag_prefix = "(", tag_suffix = ")")
plot

quartz.save(file = "fig2.pdf", type = "pdf", height = 6.5, width = 14)
```
...
As we have already emphasised, quantifying effect size heterogeneity provides the impetus for exploring why effects might vary within and across studies, while also providing context for interpreting the generality of overall effects. There are several measures commonly used to quantify effect size heterogeneity [@Borenstein2019]; however, in ecological and evolutionary meta-analysis, heterogeneity is often calculated and reported as $I_{total}^2$ as follows (if using a multilevel meta-analytic model as we describe above):
...
Supplementary information, data, code and results associated with this paper can all be found at: https://daniel1noble.github.io/nuisance_hetreogeneity/. 

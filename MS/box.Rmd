...
$$
\text{ARR} = \frac{\overline{X}_{1} - \overline{X}_{2}}{T_2 - T_1},
$$

where *T* stands for temperature in Celsius and $T_2 > T_1$, and $\overline{X}_1$ and $\overline{X}_2$ are the average physiological responses at two temperature points $T_1$ and $T_2$.

<!--- Dan we may need to use bar_X rather than just X or M for consistence in the main text --->
...
Therefore, when $X_1$ and $X_2$ are indepedent, we can obtain the sampling variance for ARR as:
...
Finally, we note that by using the basic properties of variance and the delta method, one can derive sampling variance for most of cases. Indeed, these were enough for us to derive samping variance for 'new' effect sizes (e.g. $lnQ_{10}$; see the main text) and as in Nakagawa et al. (2015, MEE) and Senior et al. (2020, RSM).
